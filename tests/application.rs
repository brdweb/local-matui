...
fn init_command_creates_configuration_without_token() {
    let dir = std::env::temp_dir().join(format!("ma-tui-cli-{}", uuid::Uuid::new_v4()));
    let path = dir.join("config.toml");
    let result = Command::new(env!("CARGO_BIN_EXE_ma-tui"))
        .arg("--init")
        .arg("--config")
        .arg(&path)
        .env_remove("MA_TUI_TOKEN")
...
fn offline_snapshot_is_real_rendering_and_explicitly_labeled() {
    let output = Command::new(env!("CARGO_BIN_EXE_ma-tui"))
        .args(["--demo", "--snapshot"])
        .env_remove("MA_TUI_TOKEN")
        .env_remove("MATUI_TOKEN")
        .output()
